...
> 前提：本仓库已经作为一个独立 Git 仓库托管在代码平台（如 GitHub / Gitee）。

1. 确保当前目录下存在 `UnityArtTool/UnityArtTool/package.json`（本项目已内置）。  
2. 在目标项目中打开 **Package Manager**：`Window -> Package Manager`。
3. 点击左上角 `+` 按钮，选择 **Add package from git URL...**。
4. 输入仓库地址，例如：
   - `https://your.git.server/yourname/unity-art-tools.git`
5. 等待 Unity 下载并导入完成后，即可在菜单栏通过 `工具/美术工具合集` 打开主工具窗口。
